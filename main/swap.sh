...
setup_swap() {
    ...
    if [ -f "$SWAP_IMG" ] && [ -f "$SWAP_FILE" ]; then
        ACTUAL_SIZE=$(stat -c %s "$SWAP_FILE" 2>/dev/null || du -b "$SWAP_FILE" | awk '{print $1}')
        if [ "$ACTUAL_SIZE" -eq "$PRECISE_BYTES" ]; then
            mkdir -p "$SWAP_MOUNT_DIR"
            if mount -o loop,rw,noatime,nodiratime,discard,barrier=0 "$SWAP_IMG" "$SWAP_MOUNT_DIR"; then
                mkswap "$SWAP_FILE" >/dev/null 2>&1
                if swapon "$SWAP_FILE" -p $SWAP_PRIORITY; then
    ...
    if su -c swapon "$SWAP_FILE" -p $SWAP_PRIORITY; then
        log "INFO" "Swap setup complete (priority: $SWAP_PRIORITY)"
        return 0
    else
        umount "$SWAP_MOUNT_DIR"
        rm -f "$SWAP_IMG"
